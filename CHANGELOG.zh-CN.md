...
## [3.1.2] - 2026-04-18 - 修复：代码块中 # 注释被错误渲染为标题

### 🐛 问题修复

- **修复代码围栏内的 `#` 注释被渲染为标题的问题**：Python 注释（如 `# ... existing code ...`）在 ` ```python ``` ` 代码块内会被错误转换为 `<h1>` 标题元素。解析器现在在其他所有解析器（标题、强调、列表、链接等）运行之前，通过占位符保护代码块内容，确保代码块内所有语法均原样保留。
- **移除表格解析器中失效的冗余代码**：原有的 `<pre>` HTML 标签检测逻辑现已完全不可达（代码块在进入表格解析前已替换为占位符），已予以删除。
- **重命名内部占位符以提升可读性**：代码块占位符改为 `MARKDOWN-BLOCK-CODE-N`，表格单元格内联代码占位符改为 `MARKDOWN-CELL-CODE-N`，彻底消除潜在命名冲突。

- 2026-04-18 - 修复：代码围栏语言标识符后含尾随空格时的渲染问题

### 🐛 问题修复

- **修复代码块语言标识符后含尾随空格时无法渲染的问题**：形如 ` ```python   `（语言标识符后有空格）的代码围栏无法被正确解析，导致 `<pre>`、`<code>` 等 HTML 标签以纯文本形式显示在 Webview 中。现已修复正则表达式，允许换行符前有任意尾随空格或制表符。

## [3.1.0] - 2026-04-14 - 资源管理器菜单与光标自动定位

### ✨ 新功能

- **资源管理器右键菜单**：在资源管理器中右键 `.md` 或 `.txt` 文件，现在可直接选择「提词器模式」。`.md` 文件还额外显示「演示模式（幻灯片）」选项。两个选项展示在同一分组中，方便查找。
- **编辑器右键菜单分组**：编辑器右键菜单中的三个 CueMode 命令（提词器模式、提词器模式（光标位置）、演示模式）现在被放入专属分组，显示在一起，不再分散在导航菜单各处。
- **切换回标签页时自动定位光标**：提词器已打开时，从编辑器切换回 CueMode 标签页，提词器会自动滚动到当前光标位置。扩展在 CueMode 激活期间持续追踪源文件中的光标转动，并在指针切换时使用最近一次的位置。

## [3.0.4] - 2026-04-14 - 修复提词器中星号字面量错误渲染

### 🛠 修复内容

- 修复提词器中星号用作字面量字符时（例如乘法算式 `5 * 3 = 15 * 2`）被错误渲染为斜体的问题。
- 将斜体识别正则更新为要求开始 `*` 后紧跟非空白字符、结束 `*` 前紧跟非空白字符，符合 CommonMark 规范行为。
- 新增 8 个回归测试，覆盖两侧带空格的星号、字面量与斜体混合内容、单个未配对星号等场景。

## [3.0.3] - 2026-04-07 - 更安全的下划线强调解析

### 🛠 修复内容

- 修复 markdown 强调解析误判，确保 `light_is_on` 这类 snake_case 标识符在 CueMode 中不再被错误渲染为斜体内容。
- 在解析层和渲染层补充回归测试，确保带下划线的标识符保持原样显示。

## [3.0.2] - 2026-04-02 - 光标感知回编与更顺滑的实时同步

### ✨ 改进

- 新增 **提词器模式（光标位置）**，可直接围绕当前编辑器光标重新进入 CueMode，从正在编辑的位置继续查看内容。
- 改进双击返回编辑器的定位逻辑：会结合渲染文本、上下文和点击偏移，更准确映射回源码中的行列位置，适用于 markdown、重复文本和中文内容。

### 🛠 修复内容

- 将 CueMode 源文档同步改为轻量增量更新，不再在每次击键时重建整个 webview HTML。
- 当 CueMode 标签页不可见时，延迟刷新到重新切回标签页时再执行，显著降低编辑器输入卡顿。
- 保持 `cuemode.openEditorAtLine` 对旧调用方式 `{ uri, line }` 的兼容性。

### 📚 文档

- 同步更新包元数据、README 亮点、GitHub Pages 文案以及仓库内当前版本说明到 3.0.2。

## [3.0.1] - 2026-03-02 - 演示模式体验修复与细节优化

### 🛠 修复内容

- 将 CueMode 提词器视图与演示模式的 markdown 换行处理逻辑解耦，确保演示模式中的段落/换行符合预期。
- 修复 markdown 表格处理中的前导空格误删问题：非表格行保留原始缩进。
- 分页提示新增“**不再提示**”持久化选项。
- 调整演示模式顶部“帮助/关闭”栏显示策略：
  - 进入演示模式后显示 5 秒；
  - 之后仅在鼠标悬停到顶部控制区域时显示；
  - 在幻灯片内容区域移动或点击鼠标不再触发顶部栏显示。

## [3.0.0] - 2026-03-01 - 演示模式正式发布与全量多语言同步

### ✨ 发布亮点

- **演示模式正式可发布**：多幻灯片工作流与快捷控制完成发布级打磨。
- **多语言全面同步**：运行时语言文件（`src/locales/*`）与 Marketplace 语言文件（`package.nls*`）在 6 种语言中全部对齐。
- **发布文档全面更新**：README、更新日志、GitHub Pages 与发布模板统一升级到 3.0.0。

### 🌐 国际化

- 补齐德语、法语、日语、韩语缺失键。
- 中文语言键集与英文基准完全对齐（含复数键与快捷键项一致性）。
- 已验证所有语言文件键集一致：0 缺失 / 0 多余。

### 📚 文档与发布流程

- 包元数据与主要文档版本号统一更新到 `3.0.0`。
- 发布检查清单与 Issue 模板示例版本同步更新。
...
- **功能文档**：`project_docs/4-header-sizes-and-comments-feature.md` 综合指南
- **使用指南**：演讲备注和演示提示的最佳实践
- **技术细节**：带代码示例的实现指南
- **未来增强**：注释相关功能的路线图
...
| 版本  | 发布日期   | 主要功能                                                                                                   |
| ----- | ---------- | ---------------------------------------------------------------------------------------------------------- |
| 3.0.3 | 2026-04-07 | **更安全的下划线强调解析** - 保持 `light_is_on` 等 snake_case 标识符原样显示，同时不影响正常 markdown 强调 |
| 3.0.2 | 2026-04-02 | **光标感知回编与更顺滑的实时同步** - 双击回源码更精准、支持从光标位置进入提词器、后台刷新更轻量            |
| 3.0.0 | 2026-03-01 | **演示模式正式发布与全量多语言同步** - 多幻灯片流程完善、6 语种键集全量对齐、文档与页面统一升级            |
