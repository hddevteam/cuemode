...
- **下划线解析更安全**：在 markdown 模式下，`light_is_on` 这类 snake_case 标识符会保持原样显示，不再被误判成斜体强调。
- **回归测试已补齐**：解析层和渲染层均已新增测试，防止这个 markdown 边界问题再次出现。

### 3.0.2 有哪些改进？

- **双击回编更精准**：在 CueMode 中双击文本时，能更准确地回到源码中的对应行列，尤其适合 markdown、重复文本和中文内容。
- **新增提词器模式（光标位置）**：可围绕当前编辑器光标重新打开提词器，从正在编辑的位置继续查看。
- **实时同步更顺滑**：源文档更新改为轻量增量同步，隐藏的 CueMode 标签页会在重新切回时再刷新，减少输入卡顿。
...
4. **从当前光标位置进入** - 当你想围绕正在编辑的位置继续查看时，使用 **提词器模式（光标位置）**，快捷键为 `Ctrl+Alt+G`（Mac 上为 `Ctrl+Cmd+G`）
5. **开始阅读** - 按空格键开始自动滚动
6. **演示时控制**：
...
- **Ctrl+Alt+G**（Mac 上 Ctrl+Cmd+G）- 打开**提词器模式（光标位置）**
